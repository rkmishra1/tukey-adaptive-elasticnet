# Tukey Adaptive Elastic Net — Simulation Study

> Simulation study, real-data benchmarks, and figures accompanying the manuscript on the **Tukey-AdEnet** estimator. For the installable R package implementing the method itself, see the [repository root](../README.md) (`tukeyAdEnet`).
...
All commands below assume your working directory is `simulation-study/`
(i.e. `cd simulation-study` from the repository root first), and that the
`tukeyAdEnet` package itself is installed, since `R/competitors.R` calls it
directly:
...
**Required packages:** `tukeyAdEnet`, `glmnet`, `rqPen`, `robustHD`, `robustbase`
...
│                           # (Tukey-AdEnet itself comes from the tukeyAdEnet
